...
fn rotate_e1_by_90_in_e12_plane() {
    ...
    assert!(
        (rotated.component(&[2]) - 1.0).abs() < 1e-12,
        "e2 component should be 1, got {}",
        rotated.component(&[2]),
    );
    assert!(
        rotated.component(&[3]).abs() < 1e-12,
...
fn rotate_preserves_orthogonal_component() {
    ...
    let rotated = transform(&e[3], &r);

    assert!(rotated.component(&[1]).abs() < 1e-12);
    assert!(rotated.component(&[2]).abs() < 1e-12);
    assert!((rotated.component(&[3]) - 1.0).abs() < 1e-12);
}
...
fn explicit_sandwich_matches_transform() {
    ...
    let explicit = (&r * &e[1] * r.rev()).grade_project(1);
    let via_transform = transform(&e[1], &r);
...
fn rotate_bivector() {
    ...
    assert!(
        (rotated.component(&[2, 3]) - 1.0).abs() < 1e-12,
        "e2^e3 component should be 1, got {}",
        rotated.component(&[2, 3]),
    );
    assert!(
        rotated.component(&[1, 3]).abs() < 1e-12,
        "e1^e3 component should vanish, got {}",
        rotated.component(&[1, 3]),
    );
}
...
fn composition_associativity() {
    ...
    let r1 = rotor(&wedge(&e[1], &e[2]), 0.3);
    let r2 = rotor(&wedge(&e[2], &e[3]), 0.5);
    let r3 = rotor(&wedge(&e[1], &e[3]), 0.7);

    let v = &(&e[1] * 1.0) + &(&(&e[2] * 2.0) + &(&e[3] * 3.0));
...
fn r_times_r_inverse_is_identity() {
    ...
    let r = rotor(&wedge(&e[1], &e[2]), 1.23);
    let v = &(&e[1] * 2.0) + &(&e[2] * 3.0);
...
fn rotor_in_4d() {
    ...
    assert!(rotated.component(&[2]).abs() < 1e-12);
    assert!((rotated.component(&[3]) - 1.0).abs() < 1e-12);
    assert!(rotated.component(&[4]).abs() < 1e-12);

    // e2 and e4 unchanged
    let rotated_e2 = transform(&e[2], &r);
    assert!((rotated_e2.component(&[2]) - 1.0).abs() < 1e-12);

    let rotated_e4 = transform(&e[4], &r);
    assert!((rotated_e4.component(&[4]) - 1.0).abs() < 1e-12);
}
